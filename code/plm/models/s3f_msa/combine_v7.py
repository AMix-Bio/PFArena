#!/usr/bin/env python3
"""Combine v7 joint EVE scores with the fresh v7 S3F run."""
...
def main() -> None:
    ...
    if args.output_dir.exists():
        raise FileExistsError(f"S3F-MSA v7 output already exists: {args.output_dir}")
    ...
    source = chain_scores.assign(local=chain_scores.eve_score_source.eq("trained_local")).groupby("sample_id")["local"].any().map({True: 0, False: 1}).rename("eve_source_code")
    ...
    if s3f_summary["dataset_hash"] != dataset["dataset_hash"] or len(s3f) != len(samples):
        raise ValueError("fresh v7 S3F run is incomplete")
    ...
    run_config = {
        ...
        "eve_sources": {"0": "locally trained five-seed model"},
        "standardization_scope": "evaluation candidates within each v7 query", "standard_deviation_correction": 1,
