...
`prepare_inputs.py` creates the EVE-formatted inputs, and `run_stage.py`
...
evaluation results are retained under `results/s3f_msa`. Re-running the public
workflow reproduces the method and configuration; bitwise-identical EVE values
are not guaranteed across GPU and CUDA stacks.
